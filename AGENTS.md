...
| **Harvest** | **Read-only, not a timeline source:** the time entries you have already logged for the selected day, shown as a strip above the feed (total hours + one row per entry). Fetched live on every day switch — never cached, because entries change after the fact. Posting entries is a separate TODO item. | Harvest API v2 personal access token + account ID (https://id.getharvest.com/developers). Every request carries `Authorization: Bearer`, `Harvest-Account-Id` and a `User-Agent` header. | ✅ Done (read) |
...
- `src/bindings.ts` — **Generated by tauri-specta from Rust commands. Do not edit.** Re-generated in debug builds via `lib.rs::run()` and also by the `typescript_bindings_export_successfully` test in `lib.rs`, so `npm test` regenerates it and catches specta-export errors (e.g. an `i64` field that needs a bigint policy).
- `src/components/Main.svelte` — Main tab content (timeline view)
- `src/components/HarvestStrip.svelte` — Compact "Logged in Harvest" strip rendered by `DayTimeline.svelte` between the date nav and the feed when Harvest is enabled: total hours for the day, one row per entry (hours, project, task, notes, running badge), inline error on fetch failure. Pure presentation; `DayTimeline` owns the `get_harvest_entries_for_day` fetch so a Harvest failure never blocks the timeline. Hour formatting (`H:MM`) and totals live in `src/lib/harvest.ts`.
- `src/components/TimelineExport.svelte` — "Export for AI" popover in the date nav: pick a day/range (with presets), choose Markdown or JSON, copy to clipboard. Formatting lives in `src/lib/export.ts` (`toMarkdown` / `toJson`); the data comes from the `export_timeline_for_range` command.
- `src/components/settings/Settings.svelte` — Settings tab shell; composes domain panels (`UI.svelte`, `Git.svelte`, …)
- `src/components/settings/*.svelte` — One panel per settings domain (theme, git, GitHub, Jira, Zulip, Calendar, Harvest, export prompt, cache). `Export.svelte` edits the custom AI-export prompt; `src/lib/export-prompt.ts` resolves the effective prompt (empty or matching a built-in default → the active-language default, so it follows the UI language; otherwise the user's custom text).
...
- `src-tauri/src/commands/settings_{domain}.rs` — One file per settings domain (e.g. `settings_ui`, `settings_git`, `settings_jira`, `settings_github`, `settings_zulip`, `settings_ical`, `settings_harvest`, `settings_export`): a single JSON document per domain in the `settings` table, `get_*` / `set_*` commands, specta types for the frontend. `settings_ical` additionally exposes `get_ical_sync_status` (returns `{ syncing, last_synced_at, last_error }`) and `trigger_ical_sync` (kicks off a background sync, returns immediately). The background sync runs via `tauri::async_runtime::spawn_blocking`, opens its **own** SQLite connection (via `db_path`) to avoid contending with the main mutex, writes parsed events into `ical_events` and updates `ical_sync_meta`, then invalidates `timeline_day_cache`. Calling `set_settings_ical` auto-triggers a sync if URLs changed or the source was just enabled.
- `src-tauri/src/commands/timeline/mod.rs` — also exposes `export_timeline_for_range(start, end)`, which returns `Vec<ExportDay { date, events }>` for every day in the inclusive range (empty days included), reusing the day cache for elapsed days and one live fetch for the rest. The shared `collect_range_events` helper (fetch all sources once over a contiguous range, bucket by local day, emit per-source progress) backs both `export_timeline_for_range` and `get_day_counts_for_month`. `get_timeline_for_day` is an `async` Tauri command that uses `tokio::task::block_in_place` to run blocking source fetches without freezing the UI. It emits `timeline:source` Tauri events (`{ source, done: bool }`) before and after each source so the frontend can show per-source loading progress. Merges results and reads/writes the per-day cache for elapsed days. `get_day_counts_for_month(year, month)` powers the datepicker heatmap: it fetches each source **once** for the whole uncached range (via `events_for_range`), buckets results per local day, and populates `timeline_day_cache` for every elapsed day in one pass — far cheaper than N per-day calls.
- `src-tauri/src/commands/timeline/{source}.rs` — One module per timeline provider (`git.rs`, `github.rs`, `ical.rs`, `jira.rs`, `zulip.rs`). Each exposes `events_for_day(state, day)` (used by `get_timeline_for_day`) and `events_for_range(state, start_day, end_day)` returning `Vec<(NaiveDate, i64, TimelineEvent)>` bucketed by local day (used by `get_day_counts_for_month`); `events_for_day` is a thin wrapper around `events_for_range`. `ical.rs` reads pre-parsed events from the `ical_events` SQLite table (populated by the background sync in `settings_ical.rs`) rather than fetching feeds at query time.
...
- `src-tauri/src/commands/harvest.rs` — Read-only Harvest API v2 client. `get_harvest_entries_for_day(day)` returns `Vec<HarvestTimeEntry>` (hours, notes, project/client/task names, `is_running`, day-view URL) for the authenticated user via `GET /v2/users/me` → `GET /v2/time_entries?user_id=&from=&to=`. Deliberately bypasses `timeline_day_cache` (entries change after the fact). Caches the user id + web `base_uri` (from `GET /v2/company`) in-process per credential pair so a day switch is one request. `settings_harvest.rs` exposes `test_settings_harvest`, which reuses this module's `fetch_current_user`.
- `src-tauri/src/commands/harvest_done.rs` — Load/save timeline Harvest checkmarks (SQLite rows keyed by UUID v5 of `TimelineEvent.id`). Still the manual toggle; deriving it from real Harvest entries is a follow-up.
...
| `RECALL_TEST_HARVEST_TOKEN` | Harvest API integration test (personal access token) |
| `RECALL_TEST_HARVEST_ACCOUNT_ID` | Harvest API integration test (numeric account ID) |
