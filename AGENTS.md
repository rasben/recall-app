...
- `npm run test:frontend` — runs `svelte-kit sync` and then Vitest for the Svelte/TypeScript side (config: `vitest.config.ts`). Test files live alongside source as `*.test.ts` under `src/`. The config loads `@sveltejs/vite-plugin-svelte`, so tests can import `*.svelte.ts` rune modules (e.g. `i18n.svelte.ts`); that plugin needs `.svelte-kit/tsconfig.json` to exist, which is why the script syncs first.
