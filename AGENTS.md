...
| **GitHub** | PRs and issues you opened, your issue/PR comments, reviews and review comments, and pushes. Opened items, comments and reviews come from the REST Search API (`/search/issues` with `author:` / `commenter:` / `reviewed-by:` and `created:` / `updated:` qualifiers, `advanced_search=true`), with the candidate issues/PRs followed up through the comments/reviews list endpoints for exact timestamps — so any date range works. Pushes come from the Events API for as far back as it reaches (~300 events / 90 days) and from `/search/commits` (default branch, by author date) beyond that. Each event type is opt-in per user; `PushEvent` is offered as an alternative to **Local git** for users (e.g. on Windows) who do not want shell-based repo scanning. Search is limited to 30 requests/minute; one range fetch issues at most three. | GitHub PAT (Personal Access Token) — username + token stored in settings. For private repos a fine-grained PAT needs read access to Contents, Issues and Pull requests. | ✅ Done |
